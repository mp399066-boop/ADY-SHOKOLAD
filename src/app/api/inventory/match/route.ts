...
export async function GET(req: NextRequest) {
  ...
  const matches = ((mats ?? []) as { id: string; שם_חומר_גלם: string; כמות_במלאי: number; יחידת_מידה: string }[])
    .filter(m => {
      if (nameMatches(name, m.שם_חומר_גלם)) return true;
      return (aliasByMaterial.get(m.id) ?? []).some(al => nameMatches(name, al));
    })
    .slice(0, 10)
    .map(m => ({ id: m.id, name: m.שם_חומר_גלם, quantity: m.כמות_במלאי, unit: m.יחידת_מידה }));
