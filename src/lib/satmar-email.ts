...
function buildHtml(order: Record<string, unknown>, customer: Record<string, string> | null, items: Record<string, unknown>[]): string {
  ...
  const itemsHtml = items.map((item) => {
    ...
    const name = item['סוג_שורה'] === 'מארז'
      ? `מארז פטיפורים ${item['גודל_מארז'] || ''}${pfNames ? ` (${pfNames})` : ''}`
  ...
  const notesSection = order['הערות_להזמנה']
    ? `<div style="background:#FFF8EE;border:1px solid #EDE0CE;border-radius:8px;padding:14px 18px;margin-bottom:20px">
        <p style="margin:0;font-size:13px;color:#5C3D22">📝 <em>${order['הערות_להזמנה']}</em></p>
      </div>`
    : '';
    ...
    <div style="background:#FFFFFF;border:1px solid #EDE0CE;border-top:none;padding:28px 32px;border-radius:0 0 12px 12px">
      ...
      <div style="background:#FAF7F0;border-radius:8px;padding:16px 20px;margin-bottom:20px">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:13px">
          <tr><td style="padding:6px 0;color:#9B7A5A;width:40%">מספר הזמנה</td><td style="padding:6px 0;font-weight:700">${order['מספר_הזמנה']}</td></tr>
          <tr><td style="padding:6px 0;color:#9B7A5A">תאריך אספקה</td><td style="padding:6px 0;font-weight:600">${order['תאריך_אספקה'] || '—'}</td></tr>
...
export async function sendSatmarSummaryEmail(
  orderId: string,
): Promise<void> {
  ...
  if (!order) return;
  if (order.satmar_summary_sent_at) return; // already sent — prevent duplicate
  ...
  const customer = order.לקוחות as Record<string, string> | null;
  const to = customer?.['אימייל'] || '';

  if (!to) {
    console.log('[satmar-email] no email on customer, skipping', orderId);
    return;
  }

  const apiKey = process.env.RESEND_API_KEY;
  if (!apiKey) {
    console.warn('[satmar-email] RESEND_API_KEY not set — skipping');
    return;
  }

  const html = buildHtml(order as Record<string, unknown>, customer, (items || []) as Record<string, unknown>[]);
  ...
  try {
    ...
    await supabase
      .from('הזמנות')
      .update({ satmar_summary_sent_at: new Date().toISOString() })
      .eq('id', orderId);

    console.log('[satmar-email] sent to', to, 'for order', order['מספר_הזמנה']);
  } catch (err) {
    console.error('[satmar-email] failed for order', orderId, ':', err instanceof Error ? err.message : err);
  }
}
